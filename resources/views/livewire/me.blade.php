        ...
        <div class="w-full max-w-md">
            ...
            <form wire:submit.prevent="updateProfile" class="space-y-4">
                <div class="relative w-[150px] h-[150px] mx-auto mt-8 mb-10">
                    @if ($photo)
                        <img class="w-full h-full rounded-full object-cover" src="{{ $photo->temporaryUrl() }}"
                            alt="Profile Picture">
                    @elseif($currentPhoto)
                        <img class="w-full h-full rounded-full object-cover"
                            src="{{ asset('storage/images/profiles/' . $currentPhoto) }}" alt="Profile Picture">
                    @else
                        <img class="w-full h-full rounded-full object-cover" src="https://placehold.co/150"
                            alt="Profile Picture">
                    @endif
                    <input type="file" id="profilePhoto" accept="image/*" class="hidden" wire:model="photo">
                    <button type="button" onclick="document.getElementById('profilePhoto').click()"
                        class="absolute bottom-0 right-0 w-[45px] h-[43px] bg-[#D9D9D9] rounded-full flex items-center justify-center border-2 border-white hover:bg-gray-300 transition-colors">
                ...
                <div wire:loading wire:target="photo" class="text-center">
                    <span class="text-sm text-gray-600">Uploading photo...</span>
                </div>
                <div>
            ...
            <div class="flex justify-center items-center mt-6">
                <button wire:click="logout"
                    class="cursor-pointer flex items-center space-x-2 text-black hover:text-red-600 transition-colors duration-200">
